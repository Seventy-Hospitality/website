/**
 * Pure reservation-lifecycle logic shared by the M4 detail / edit / cancel
 * surfaces (M2 home reuses the respond transition for its inline
 * accept/decline). Ported 1:1 from member-web/src/lib/reservation-policy.ts,
 * mirroring the backend rules in api/lib/contexts/bookings/domain: the server
 * stays the source of truth, these mirrors only drive previews and optimistic
 * UI. The slot/date math comes from M3's booking helpers so the two clients
 * agree slot-for-slot and cent-for-cent.
 */
import type {
  Reservation,
...
import { minutesToTimeLabel, slotsFromRange, timeLabelToMinutes } from '../reserve/booking';
 ...
 * incremental charge collected via the native PaymentSheet before the move
